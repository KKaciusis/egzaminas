...
function App() {
  ...
  const sortByTime = () => {
    ...
    planesCopy = planesCopy.sort((a, b) => (b.arrival_time).localeCompare(a.arrival_time))
    setPlanes(planesCopy);
  }
  const sortByTimeUp = () => {
    let planesCopy = planes.slice()
    planesCopy = planesCopy.sort((a, b) => (a.arrival_time).localeCompare(b.arrival_time))
    setPlanes(planesCopy);
  }
  const sortByTown = () => {
    let planesCopy = planes.slice()
    planesCopy = planesCopy.sort((a, b) => (b.from_town).localeCompare(a.from_town))
    setPlanes(planesCopy);
  }
  const sortByTownUp = () => {
    let planesCopy = planes.slice()
    planesCopy = planesCopy.sort((a, b) => (a.from_town).localeCompare(b.from_town))
    setPlanes(planesCopy);
  }
  const sortByLate = () => {
    let planesCopy = planes.slice()
  ...
  return (
    <>
      <TopRow latePlanes={latePlanes} notLatePlanes={notLatePlanes} totalPlanes={totalPlanes} sortByNotLate={sortByNotLate} sortByLate={sortByLate} sortByTownUp={sortByTownUp} sortByTown={sortByTown} sortByTime={sortByTime} sortByTimeUp={sortByTimeUp} setIsOpen={setIsOpen} />
      <Router>
        <Switch>
